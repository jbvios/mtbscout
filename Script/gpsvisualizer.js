...
google_api_version = gv_api_version = FindGoogleAPIVersion(); if (!self.GLatLng) { google_api_version = 0; }
...
if (self.gv_maptypecontrol_style) { // this indicates a pre-gv_options version of the code
	if ($('gv_credit') && $('gv_credit').innerHTML.indexOf('gpsvisualizer.com') < 0) {
		...
		credit_div.style.font = '10px Verdana,sans-serif'; credit_div.style.padding = '1px'; credit_div.style.backgroundColor = '#ffffff';
...
function GV_Setup_Global_Variables() {
	...
	if (!gv_options['map'] || typeof(gv_options['map']) == 'function') { gv_options['map'] = 'gmap'; }
	
	// Define parameters of different marker types
	if (!self.gv_icons) { gv_icons = []; }
	...
	gv_marker_list_exists = null; gv_marker_list_html = ''; gv_marker_list_folders = []; gv_marker_list_count = 0;
	...
	gv_marker_list_text_tooltip = ''; gv_marker_list_icon_tooltip = '';
	if (self.gv_marker_list_options) {
		gv_marker_list_html = ''; gv_marker_list_folders = [];
	...
	if (gv_options['default_marker']['icon'].indexOf('/') > -1) {
		...
		gv_default_icon_object.shadow = null; gv_default_icon_object.shadowSize = null;
		...
		if (!gv_icons[gv_options['default_marker']['icon']]) { gv_options['default_marker']['icon'] = 'googlemini'; }
		...
		if (gv_options['marker_shadows'] === false) {
			gv_default_icon_object.shadow = null; gv_default_icon_object.shadowSize = null; gv_default_icon_object.infoShadowAnchor = null;
		} else {
		...
		if (gv_icons[gv_options['default_marker']['icon']]['im']) {
			...
			for (var i=0; i<gv_icons[gv_options['default_marker']['icon']]['im'].length; i++) { gv_default_icon_object.imageMap[i] = gv_icons[gv_options['default_marker']['icon']]['im'][i]; }
		} else {
	...
	if (gv_options['default_marker']['scale'] && gv_options['default_marker']['scale'] != 1) {
		...
		gv_default_icon_object.iconSize.width *= sc; gv_default_icon_object.iconSize.height *= sc;
		gv_default_icon_object.iconAnchor.x *= sc; gv_default_icon_object.iconAnchor.y *= sc;
		gv_default_icon_object.infoWindowAnchor = new GPoint(gv_default_icon_object.iconSize.width*0.75,0);
		gv_default_icon_object.infoShadowAnchor = new GPoint(gv_default_icon_object.iconSize.width*0.5,gv_default_icon_object.iconSize.height);
		if (gv_default_icon_object.shadow) {
			gv_default_icon_object.shadowSize.width *= sc; gv_default_icon_object.shadowSize.height *= sc;
		}
		if (gv_default_icon_object.imageMap) { for (var i=0; i<gv_default_icon_object.imageMap.length; i++) { gv_default_icon_object.imageMap[i] *= sc; } }
	}
...
function GV_Setup_Map(opts) { // opts = options hash
	if (!opts && !self.gv_options) { return false; } else if (!opts && self.gv_options) { opts = gv_options; }
	if (!opts['map'] || typeof(opts['map']) == 'function') { opts['map'] = 'gmap'; }
	var map_name = opts['map']; var map = eval(map_name); if (!map) { return false; }
	...
	else { gv_marker_list_options = []; } // it needs to be created or the next statements will throw errors
	...
	if (opts['full_screen']) {
		...
		if (opts['width']) { map.getContainer().style.width = parseFloat(opts['width'])+'px'; }
		if (opts['height']) {
			...
			if ($('gv_marker_list_static') || (opts['marker_list_options'] && opts['marker_list_options']['id_static'] && $(opts['marker_list_options']['id_static'])) ) {
				...
				if ($(marker_list_id).style && !$(marker_list_id).style.height) { $(marker_list_id).style.height = parseFloat(opts['height'])+'px'; }
			}
		}
		map.checkResize();
	}
	...
	if (typeof(opts['zoom']) == 'undefined') { opts['zoom'] = 'auto'; }
	if (opts['zoom'] == 'auto' || (!opts['zoom'] && opts['zoom'] != '0')) {
		if (!opts['center'] || (opts['center'] && opts['center'].length == 0)) { opts['center'] = [40,-100]; }
	...
	if (self.gv_marker_list_options && gv_marker_list_exists) {
		...
		if (o['floating'] && o['container_id'] && $(o['container_id'])) {
			if (o['width']) { $(o['id']).style.width = o['width']+'px'; }
			if (o['height']) { $(o['id']).style.height = o['height']+'px'; }
		...
		if (o['help_tooltips']) {
			gv_marker_list_text_tooltip = 'Click to '; gv_marker_list_icon_tooltip = 'Click to ';
			if (o['center']) { gv_marker_list_text_tooltip += 'center + '; gv_marker_list_icon_tooltip += 'center + '; }
			if (o['zoom']) { gv_marker_list_text_tooltip += 'zoom in + '; gv_marker_list_icon_tooltip += 'zoom in + '; }
			if (o['toggle']) { gv_marker_list_text_tooltip += 'hide/show the marker + '; }
			if (o['url_links'] && m.url) { gv_marker_list_text_tooltip += 'open the marker\'s link + '; }
			if (o['info_window'] && !o['toggle']) { gv_marker_list_text_tooltip += 'open the info window'; }
			if (o['info_window']) { gv_marker_list_icon_tooltip += 'open the info window'; }
	...
	if (opts['searchbox_options'] == true) { opts['searchbox_options'] = {searchbox:true}; }
	...
	if (opts['mouse_coordinates']) {
		...
		if (!$('gv_mouse_container')) {
			var mouse_div = document.createElement('div'); mouse_div.id = 'gv_mouse_container';
	...
	if (opts['measurement_tools'] == true) { opts['measurement_tools'] = {enabled:true}; }
...
function GV_Place_Measurement_Tools(opts) {
	...
	if (o['distance'] !== false) { o['distance'] = true; }
	if (o['area'] !== false) { o['area'] = true; }
	if (!self.gmap || (!o['distance'] && !o['area'])) { return false; }
...
function GV_Place_Draggable_Box(map,id,position,draggable,collapsible) {
	...
	if ($(container_id) && position && position.length >= 3) {
		...
		if ($(table_id) && $(handle_id) && draggable) {
			...
			if ($(handle_id)) { $(handle_id).style.display = 'none'; }
		}
	}
}
	...
	if (eval('self.'+marker_array_name)) {
		...
		if (!gv_waypoint_events_are_set_up) {
			if (gv_filter_waypoints) {
				GEvent.addListener(map, "moveend", function() { window.setTimeout('GV_Process_Markers('+map_name+','+marker_array_name+')',50) });
				GEvent.addListener(map, "resize", function() { window.setTimeout('GV_Process_Markers('+map_name+','+marker_array_name+')',50) });
			}
			...
			if (gv_options['dynamic_data'] && gv_options['dynamic_data'].length) {
				for (var i=0; i<gv_options['dynamic_data'].length; i++) {
					if (gv_options['dynamic_data'][i] && gv_options['dynamic_data'][i]['url'] && gv_options['dynamic_data'][i]['reload_on_move']) {
						...
						if (gv_options['zoom'] && gv_options['zoom'] == 'auto') { gv_options['zoom'] = 8; } // important! otherwise it goes into a zoom/move infinite loop
						var func = 'GV_Reload_Markers_From_File('+i+')';
						GEvent.addListener(map, "moveend", function() { window.setTimeout(func,50) });
					}
				}
			}
...
function GV_Setup_Waypoint_Processing_Events(map_name,marker_array_name) { GV_Setup_Marker_Processing_Events(map_name,marker_array_name); }
...
function GV_Process_Waypoints(map,marker_array) { GV_Process_Markers(map,marker_array); }

gv_dynamic_file_index = -1; gv_reloading_data = false; gv_static_marker_count = -1; // global variables that need to be set just once
...
function GV_Finish_Map(opts) {
	...
	var marker_array_name = (opts['marker_array']) ? opts['marker_array'] : 'wpts';
	var marker_array = eval('self.'+marker_array_name); if (!marker_array) { eval(marker_array_name+' = []'); }
	var track_array_name = (opts['track_array']) ? opts['track_array'] : 'trk';
	var track_info_array_name = (opts['track_info_array']) ? opts['track_info_array'] : 'trk_info';
	var track_segments_array_name = (opts['track_segments_array']) ? opts['track_segments_array'] : 'trk_segments';
	...
	if (self.gv_static_marker_count < 0) { gv_static_marker_count = (marker_array) ? marker_array.length : 0; }
	...
	gv_dynamic_data_last_center = map.getCenter(); gv_dynamic_data_last_zoom = map.getZoom(); // for reload-on-move purposes
...
function GV_Autozoom() { // automatically adjust the map's zoom level to cover the elements passed to this function
	...
	var opts = arguments[0]; arguments.shift;
	...
	var map = eval(opts['map']); if (!map) { return false; }
	var min_lat = 90; var max_lat = -90; var min_lon = 360; var max_lon = -360;
	for (var i=0; i<arguments.length; i++) {
		if (arguments[i]) {
			if (arguments[i].getLatLng) { // it's a waypoint
				if (arguments[i].getLatLng().lng() < min_lon) { min_lon = arguments[i].getLatLng().lng(); }
				if (arguments[i].getLatLng().lng() > max_lon) { max_lon = arguments[i].getLatLng().lng(); }
				if (arguments[i].getLatLng().lat() < min_lat) { min_lat = arguments[i].getLatLng().lat(); }
				if (arguments[i].getLatLng().lat() > max_lat) { max_lat = arguments[i].getLatLng().lat(); }
				...
				if (arguments[i][0] && arguments[i][0].getVertexCount) { // it's a single track
					for (ts=0; ts<arguments[i].length; ts++) { // tracks are collections of track segments
						...
						if (bounds) {
							var w = bounds.getSouthWest().lng(); var s = bounds.getSouthWest().lat(); var e = bounds.getNorthEast().lng(); var n = bounds.getNorthEast().lat();
					...
					for (var j=0; j<arguments[i].length; j++) { // it's a collection of tracks or waypoints
						if (arguments[i][j]) {
							if (arguments[i][j][0] && arguments[i][j][0].getVertexCount) { // it's a track
								...
								if (arguments[i][j].getLatLng().lng() < min_lon) { min_lon = arguments[i][j].getLatLng().lng(); }
								if (arguments[i][j].getLatLng().lng() > max_lon) { max_lon = arguments[i][j].getLatLng().lng(); }
								if (arguments[i][j].getLatLng().lat() < min_lat) { min_lat = arguments[i][j].getLatLng().lat(); }
								if (arguments[i][j].getLatLng().lat() > max_lat) { max_lat = arguments[i][j].getLatLng().lat(); }
							}
						}
					}
				}
	...
	if (min_lat != 90 && max_lat != -90 && min_lon != 360 && max_lon != -360) {
		...
		if (min_lat == max_lat && min_lon == max_lon) { zoom = (opts['default']); } // arbitrary zoom for single-point maps
		...
		if (opts['save_position']) { map.savePosition(); }
	}
}


var gv_marker_count = 0;
if (!self.gv_options) { gv_options = new Array(); gv_options['map'] = null; } // turns out ".map" is a predefined method!
...
function GV_Marker(map,marker_info,lon,name,desc,url,color,style,width,label_id) {
	...
	if (m['icon_url'] || (m['icon'] && m['icon'].toString().indexOf('/') > -1) || (gv_garmin_icons && gv_garmin_icons[m['icon']])) {
		if (gv_garmin_icons && gv_garmin_icons[m['icon']] && gv_garmin_icons[m['icon']]['url']) {
			...
			var anchor_x = 7; var anchor_y = 7;
			if (gv_garmin_icons[m['icon']]['anchor'] && gv_garmin_icons[m['icon']]['anchor'].length == 2) {
				anchor_x = gv_garmin_icons[m['icon']]['anchor'][0]; anchor_y = gv_garmin_icons[m['icon']]['anchor'][1];
			}
			...
			var default_width = 32; var default_height = 32;
				if (tempIcon.image.match(/chart\.apis\.google\.com\/.*\bch\w+=[\w_]*pin\b/)) { default_width = 21; default_height = 34; } // is there a more efficient way to do this??
				default_width = (self.gv_marker_icon_size && gv_marker_icon_size[0]) ? gv_marker_icon_size[0] : default_width; default_height = (self.gv_marker_icon_size && gv_marker_icon_size[1]) ? gv_marker_icon_size[1] : default_height;
			...
			var default_anchor_x = tempIcon.iconSize.width*0.5; var default_anchor_y = tempIcon.iconSize.height*0.5;
				if (tempIcon.image.match(/chart\.apis\.google\.com\/.*\bch\w+=[\w_]*pin\b/)) { default_anchor_x = 10; default_anchor_y = 33; } // is there a more efficient way to do this??
				default_anchor_x = (self.gv_marker_icon_anchor && gv_marker_icon_anchor[0] != null) ? gv_marker_icon_anchor[0] : default_anchor_x; default_anchor_y = (self.gv_marker_icon_anchor && gv_marker_icon_anchor[1] != null) ? gv_marker_icon_anchor[1] : default_anchor_y;
		...
		tempIcon.shadow = null; tempIcon.shadowSize = null; tempIcon.infoShadowAnchor = null;
		...
		var color = (m['color']) ? m['color'].toLowerCase() : gv_options['default_marker']['color'].toLowerCase(); if (color.substring(0,1) == '#') { color = color.replace(/^\#/,''); }
		...
		if (icon != gv_options['default_marker']['icon'] || custom_scale) { // these only need to be messed with if they're not using the default icon or if a scale has been specified; otherwise, they were set as part of "gv_default_icon_object"
			...
			if (custom_scale) { tempIcon.imageMap = []; for (var j=0; j<gv_icons[icon]['im'].length; j++) { tempIcon.imageMap[j] = gv_icons[icon]['im'][j]*scale; } }
	...
	if (m['no_window']) {
		...
		if (m['name']) {
			if (m['url'] && m['url'] != null) { iw_html = iw_html + '<div class="gv_marker_info_window_name"><b><a '+target_attribute+' href="'+url_quoted+'" title="'+url_quoted+'">'+m['name']+'</a></b></div>'; }
			else { iw_html = iw_html + '<div class="gv_marker_info_window_name">'+m['name']+'</div>'; }
		}
		if (m['thumbnail'] && !m['photo']) {
			...
			if (m['url']) { thumbnail = '<a '+target_attribute+' href="'+url_quoted+'">'+thumbnail+'</A>'; }
		...
		if (marker_window_width > 0 && marker_window_width < gv_options['info_window_width_maximum']) { window_width = marker_window_width; }
		else if (gv_options['info_window_width'] > 0 && gv_options['info_window_width'] < gv_options['info_window_width_maximum']) { window_width = gv_options['info_window_width']; }
	...
	if (m['label'] || m['label_id']) { // draw a permanent label
		...
		if (label_text != '') {
			...
			var offset_x = gv_options['label_offset'][0]; var offset_y = gv_options['label_offset'][1]; var label_centered = gv_options['label_centered']; var label_left = gv_options['label_left'];
			if (m['label_offset'] && m['label_offset'].length > 1) { offset_x = m['label_offset'][0]; offset_y = m['label_offset'][1]; }
			if ((m['label_centered'] == true && !label_centered) || (m['label_centered'] === false && label_centered)) { label_centered = m['label_centered']; }
			if ((m['label_left'] == true && !label_left) || (m['label_left'] === false && label_left)) { label_left = m['label_left']; label_centered = false; }
	...
	if (m['name'] || m['thumbnail']) {
		if (google_api_version >= 2) {
			...
			if (!$('gv_marker_tooltip')) { gv_marker_tooltip = GV_Initialize_Marker_Tooltip(map); } // initialize it if it hasn't been done yet
			var tooltip_html = m['name']+' ';
			if (m['thumbnail']) {
				var tn_style = (m['thumbnail_width']) ? ' style="width:'+parseFloat(m['thumbnail_width'])+'px;"' : (gv_options['thumbnail_width'] > 0) ? ' style="width:'+gv_options['thumbnail_width']+'px;"' : '';
				tooltip_html = tooltip_html + '<img class="gv_marker_thumbnail" src="'+m['thumbnail']+'"'+tn_style+'>';
			}
			if (m['photo']) { tooltip_html = tooltip_html + '<img class="gv_marker_photo" src="'+m['photo']+'">'; } // photo is hidden in tooltip but gets pre-loaded!
...
function GV_Create_Track_Tooltip(map,info) {
	// copied almost verbatim from http://www.econym.demon.co.uk/googlemaps/tooltips4.htm
	if (!self.gv_track_tooltip || !info || (!info.bounds && !info.center) || !info.name) { return false; }
...
function GV_Marker_List() {
	if (gv_marker_list_exists) {
		...
		var folders = ''; gv_marker_list_folder_state = []; gv_marker_list_folder_name = [];
		if (self.gv_marker_list_folders) {
			...
			for (var folder_name in gv_marker_list_folders) {
				folder_count += 1;
				var c = (all_collapsed) ? true : false;  var h = false;
				...
				if (typeof(gv_marker_list_options['folder_name_click']) != 'undefined') {
					if (gv_marker_list_options['folder_name_click'].match(/coll/i)) { folder_name_onclick = collapse_onclick; folder_name_message = toggle_message; }
					else if (gv_marker_list_options['folder_name_click'].match(/toggle|vis|viz/i)) { folder_name_onclick = toggle_onclick; folder_name_message = toggle_message; }
					else if (gv_marker_list_options['folder_name_click'] === false || gv_marker_list_options['folder_name_click'].match(/no/i)) { folder_name_onclick = ''; folder_name_message = ''; }
				}
		...
		if (gv_options['marker_list_options'] && gv_options['marker_list_options']['collapsed_folders'] && gv_options['marker_list_options']['collapsed_folders'].length) {
			for (var i=0; i<gv_options['marker_list_options']['collapsed_folders'].length; i++) { GV_Folder_Collapse_Toggle(gv_options['marker_list_options']['collapsed_folders'][i],false); }
		}
		if (gv_options['marker_list_options'] && gv_options['marker_list_options']['hidden_folders'] && gv_options['marker_list_options']['hidden_folders'].length) {
			for (var i=0; i<gv_options['marker_list_options']['hidden_folders'].length; i++) { GV_Folder_Visibility_Toggle(gv_options['marker_list_options']['hidden_folders'][i],false); }
		}
	}
...
function GV_Marker_List_Item(m,map_name,marker_name) {
	...
	else if (gv_marker_list_options['zoom']) { zoom_in = map_name+'.zoomIn(); '; }
	...
	if (!gv_marker_list_options['wrap_names']) { css_wrap_style = 'white-space:nowrap; '; table_wrap_style ='nowrap'; }
...
function GV_Toggle_All_Labels(marker_array_name,force_show) {
	if (!marker_array_name) { marker_array_name = 'wpts'; }
	var visible = null;
...
function GV_Draw_Track(t) {
	...
	var trk_width = (trk_info[t]['width']) ? parseFloat(trk_info[t]['width']) : 3; if (trk_width <= 0.1) { trk_width = 0; }
	...
	var bounds = new GLatLngBounds(); var lat_sum = 0; var lon_sum = 0; var point_count = 0;
	...
	for (s=0; s<trk_segments[t].length; s++) {
		if (trk_segments[t][s]['points'].length > 0) {
			...
			for (p=0; p<trk_segments[t][s]['points'].length; p++) {
				...
				lat_sum += trk_segments[t][s]['points'][p][0]; lon_sum += trk_segments[t][s]['points'][p][1]; point_count += 1;
			}
		}
	...
	if (!trk_info[t]['bounds']) { trk_info[t]['bounds'] = { w:bounds.getSouthWest().lng(), s:bounds.getSouthWest().lat(), e:bounds.getNorthEast().lng(), n:bounds.getNorthEast().lat() }; }
	if (!trk_info[t]['center'] && point_count > 0) { trk_info[t]['center'] = { lon:(lon_sum/point_count), lat:(lat_sum/point_count) }; }
	if (!$('gv_track_tooltip')) { gv_track_tooltip = GV_Initialize_Track_Tooltip(gmap); } // initialize it if it hasn't been done yet
...
function GV_Finish_Track(trk_index) {
	if (!self.trk || !self.trk_info || !self.gmap) { return false; }
	if (!$('gv_track_tooltip')) { gv_track_tooltip = GV_Initialize_Track_Tooltip(gmap); } // initialize it if it hasn't been done yet
...
function GV_Make_Track_Clickable(trk_index) {
	for (var i=0; i<trk[trk_index].length; i++) {
		...
		trk[trk_index][i].click_listener = GEvent.addListener(trk[trk_index][i], "click", function(coords){ gmap.openInfoWindowHtml(coords, html); });
		...
		trk[trk_index][i].mouseover_listener = GEvent.addListener(trk[trk_index][i], "mouseover", function(){}); // change the cursor but don't DO anything
	}
...
function GV_Area_Calculation_Info_Window(overlay,coords) {
	...
	var html = ''; var area_sqm = 0;
	if (overlay.getArea) {
		area_sqm = overlay.getArea();
	} else {
		var v = []; for (i=0;i<overlay.getVertexCount();i++) { v.push(overlay.getVertex(i)); } var temporary_polygon = new GPolygon(v,'#ffffff',1,0.1,'#ffffff',0.05); gmap.addOverlay(temporary_polygon);
		area_sqm = temporary_polygon.getArea();
		gmap.removeOverlay(temporary_polygon);
	}
	var area_sqkm = area_sqm / 1000000; var area_sqmi = area_sqm / 2589988.11;
	var area_ha = area_sqm / 10000; var area_ac = area_sqm / 4046.85642;
...
GV_Measurements = new function() {
	...
	this.Edit = function(key) {
		...
		if (this.Listener['lineupdated']) { GEvent.removeListener(this.Listener['lineupdated']); }
		if (this.Listener['singlerightclick']) { GEvent.removeListener(this.Listener['singlerightclick']); }
		if (this.Listener['endline']) { GEvent.removeListener(this.Listener['endline']); }
		...
		if (this.LinkBox[key]) { this.LinkBox[key].innerHTML = this.Text[key]['Cancel']; }
	}
	this.Cancel = function(key) {
		if (this.Overlay[key] && this.Overlay[key].getVertexCount() == 0) { this.Delete(key); return false; }
		if (this.Overlay[key]) { this.Overlay[key].disableEditing(); }
		if (this.LinkBox[key]) { this.LinkBox[key].innerHTML = this.Text[key]['Edit']; }
	}
	this.Delete = function(key) {
		if (this.Overlay[key]) { this.Overlay[key].disableEditing(); gmap.removeOverlay(this.Overlay[key]); }
 		this.Overlay[key] = null;
		if (this.LinkBox[key]) { this.LinkBox[key].innerHTML = this.Text[key]['New']; }
		if (this.ResultBox[key]) { this.ResultBox[key].innerHTML = ''; }
	}
	this.Calculate = function(key) {
		...
		if (this.Overlay[key] && this.Overlay[key].getVertexCount() > 1) {
			if (key == 'distance') {
				var meters = 0;
				for(i=1;i<this.Overlay[key].getVertexCount();i++) { meters += this.Overlay[key].getVertex(i-1).distanceFrom(this.Overlay[key].getVertex(i)); }
				if (meters < 304.8) {
		...
		if (result && this.ResultBox[key]) { this.ResultBox[key].innerHTML = result; }
	}
}
...
function GV_Add_Track_to_Tracklist(opts) {
	if (!opts || !opts['name'] || !$('gv_tracklist_links')) { return false; }
	...
	if (!$('gv_track_tooltip')) { gv_track_tooltip = GV_Initialize_Track_Tooltip(map); } // initialize it if it hasn't been done yet
	...
	if (gv_options['tracklist_options'] && gv_options['tracklist_options']['zoom_links'] !== false) {
		...
		if (opts['bounds'] && (opts['bounds']['n'] || opts['bounds']['s']) && (opts['bounds']['e'] || opts['bounds']['w'])) {
			...
			var center_lat = bounds.getCenter().lat(); var center_lon = bounds.getCenter().lng();
...
function GV_Finish_Tracklist() {
	if (!$('gv_tracklist_links')) { return false; }
...
function GV_Toggle_Overlays(map,overlay_array,force) {
	if (!map || !overlay_array) { return false; }
	if (google_api_version >= 2) {
...
function GV_Toggle_Label_Opacity(id,original_color,force) { // for track labels in the tracklist
	...
	var t = eval(id); if (!t.length) { return false; } // t is the track array itself
	if ((!t.gv_hidden && force) || (t.gv_hidden && force === false)) { return false; }
	if (t.gv_hidden) { label.style.color = gv_dimmed_color; }
	...
	if (index.toString().match(/^\d+$/)) {
		...
		for (var j=1; j<gv_marker_list_folder_name.length; j++) {
			if (index == gv_marker_list_folder_name[j]) {
				fn = j; j = gv_marker_list_folder_name.length + 1; // break out of the loop
			}
		}
	}
	return (fn);
...
function GV_Track_OutOfRange (map,trk_array,is_oor) {
	if (is_oor) {
		if (!trk_array.gv_hidden) { // if it's already hidden, there's nothing to do
			for (var j in trk_array) { if (trk_array[j].getLatLng || trk_array[j].getVertexCount) { map.removeOverlay(trk_array[j]); } }
		}
		...
		if (trk_array.gv_oor) { // only add it if it was previously out of range
			if (!trk_array.gv_hidden) { // if it's supposed to be hidden, don't show it
				for (var j in trk_array) { if (trk_array[j].getLatLng || trk_array[j].getVertexCount) { map.addOverlay(trk_array[j]); } }
			}
		}
...
function GV_Remove_All_Markers (opts) {
	if (!opts) { opts = {}; } // in case NOTHING is passed into the function
	var map_name = (opts['map'] && typeof(opts['map']) !== 'function') ? opts['map'] : 'gmap'; var map = eval(map_name); if (!map) { return false; }
	var marker_array_name = (opts['marker_array']) ? opts['marker_array'] : 'wpts'; var marker_array = eval(marker_array_name); if (!marker_array) { return false; }
	for (var j=0; j<marker_array.length; j++) {
		var w = marker_array[j];
		map.removeOverlay(w);
		if (w.label_object) { map.removeOverlay(w.label_object); }
	}
	if (gv_marker_list_exists) {
		gv_marker_list_html = ''; gv_marker_list_folders = []; gv_marker_list_count = 0;
		GV_Marker_List();
	}
}
...
function GV_Filter_Markers_With_Text (opts) {
	...
	for (var j=0; j<marker_array.length; j++) {
		...
		if (field == 'latitude') { text = w.coords.lat().toString(); }
		else if (field == 'longitude') { text = w.coords.lng().toString(); }
		else if (field == 'coords') { text = w.coords.toString(); }
		else if (field == 'namedesc') { text = w.name + "\t" + w.desc; }
		else if (w[field]) { text = w[field]; }
		if ((pattern == '' || (simple_match && text == pattern) || (!simple_match && text.match(pattern_regexp))) && !w.gv_hidden && !w.gv_oor) {
			...
			if (new_bounds) { new_bounds.extend(w.coords); } else { new_bounds = new GLatLngBounds(w.coords,w.coords); }
		} else {
...
function GV_Toggle_Markers_With_Text(opts) {
	...
	for (var j=0; j<to_be_changed.length; j++) {
		...
		if (visible && !w.gv_hidden && !w.gv_oor) {
			map.addOverlay(w);
			if (w.label_object) { map.addOverlay(w.label_object); }
			// w.gv_hidden = false;
		} else {
			map.removeOverlay(w);
			if (w.label_object) { map.removeOverlay(w.label_object); }
...
function GV_Toggle_Waypoints_With_Text (opts) { GV_Toggle_Markers_With_Text(opts); }
...
function GV_Filter_Waypoints_In_View (map,marker_array) { GV_Filter_Markers_In_View(map,marker_array); }
...
function GV_Recenter_Per_URL(opts) {
	...
	var map = eval('self.'+map_name); if (map == null) { if (self.gmap) { map = gmap; } else { return false; } }
		...
		if (center_match && center_match[1]) {
			...
			if (c.match(/^wpts?\[/)) {
				if (eval('self.'+c) && eval('self.'+c+'.coords')) {
					...
					if (open_info_window) { window.setTimeout('GEvent.trigger('+c+',"click")',500); }
				}
			} else if (c.match(/^trk?\[/)) {
				if (eval('self.'+c)) {
					GV_Autozoom({map:map_name,adjustment:0},eval(c));
					if (new_zoom) { map.setZoom(new_zoom); }
					return true;
				}
			} else {
				...
				if (c && !new_center) { // the GV_Marker_Coordinates function can detect both marker names and numeric coordinates; this was neither, so geocode it
					...
					if (opts['custom_function']) {
						...
						geocoder.getLatLng(c, function(coords){ map.setCenter(coords); } );
					}
				}
			}
		}
	...
	if (new_center) {
		...
		if (center_window_html) { map.openInfoWindowHtml(map.getCenter(),center_window_html); }
		return true;
	} else if (new_zoom) {
...
function GV_Marker_Coordinates(opts) {
	if (!opts) { return false; }
	var map_name = (opts['map'] && typeof(opts['map']) !== 'function') ? opts['map'] : 'gmap'; var map = eval(map_name); if (!map) { return false; }
	var marker_array_name = (opts['marker_array']) ? opts['marker_array'] : 'wpts'; var marker_array = eval('self.'+marker_array_name); if (marker_array == null) { return false; }
	var partial_match = (opts['partial_match'] === false) ? false : true;
	var open_info_window = (opts['open_info_window']) ? true : false;
		...
		if (partial_match) {
			for (var i=0; i<marker_array.length; i++) { // the "!new_center" clause prevents it from finding multiple markers
				if (!new_center && marker_array[i][field].toLowerCase().indexOf(pattern.toLowerCase()) > -1) {
					new_center = marker_array[i].coords; hide_crosshair = true;
					// the info window is opened HERE, in this informational subroutine, because this is where we know which marker to pop open
					if (open_info_window) { window.setTimeout('GEvent.trigger('+marker_array_name+'['+i+'],"click")',500); }
				}
			}
		} else {
			for (var i=0; i<marker_array.length; i++) {
...
function GV_Center_On_Marker(pattern_or_opts,opts) {
	...
	var map_name = (opts && opts['map']) ? opts['map'] : 'gmap'; var map = eval('self.'+map_name); if (!map) { return false; }
	var marker_array_name = (opts && opts['marker_array']) ? opts['marker_array'] : 'wpts'; var marker_array = eval('self.'+marker_array_name); if (marker_array == null) { return false; }
...
function GV_Center_On_Address(opts) {
	var map = eval('self.'+opts['map']); if (map == null) { if (self.gmap) { map = gmap; opts['map'] = 'gmap'; } else { return false; } }
	if (!$(opts['input_box'])) { return false; }
...
function GV_Center_On_Address2(info) {
	var map = eval('self.'+gv_center_on_address['map']); if (map == null) { if (self.gmap) { map = gmap; gv_center_on_address['map'] = 'gmap'; } else { return false; } }
	...
	if(info && info.Placemark && info.Placemark[0].Point) {
		...
		if (info.Placemark[0].AddressDetails) {
			if (info.Placemark[0].address.indexOf(',') > -1 && accuracy == 1) { accuracy = undefined; } // countries don't have commas in them; therefore it wasn't really a country that the geocoder found!
...
function GV_Background_Opacity(map,opacity) {
	...
	if (!$(id)) {
		...
		if (useOpacity) { new_screen.style.opacity = screen_opacity; }
		if (useFilter) { new_screen.style.filter = "alpha(opacity="+screen_opacity*100+")"; }
...
function GV_Update_Background_Screen(map,id,screen_opacity) {
	if (map && $(id) && screen_opacity != null) {
		...
		if (useOpacity) { screen.style.opacity = screen_opacity; }
		if (useFilter) { screen.style.filter = "alpha(opacity="+screen_opacity*100+")"; }
...
function GV_Define_Named_Colors() {
	...
	c['khaki'] = '#f0e68c';
	c['lavender'] = '#e6e6fa'; c['lavenderblush'] = '#fff0f5'; c['lawngreen'] = '#7cfc00'; c['lemonchiffon'] = '#fffacd'; c['lightblue'] = '#add8e6'; c['lightcoral'] = '#f08080'; c['lightcyan'] = '#e0ffff'; c['lightgoldenrodyellow'] = '#fafad2'; c['lightgreen'] = '#90ee90'; c['lightgrey'] = '#d3d3d3'; c['lightpink'] = '#ffb6c1'; c['lightsalmon'] = '#ffa07a'; c['lightseagreen'] = '#20b2aa'; c['lightskyblue'] = '#87cefa'; c['lightslategray'] = '#778899'; c['lightsteelblue'] = '#b0c4de'; c['lightyellow'] = '#ffffe0'; c['lime'] = '#00ff00'; c['limegreen'] = '#32cd32'; c['linen'] = '#faf0e6';
	...
	c['saddlebrown'] = '#8b4513'; c['salmon'] = '#fa8072'; c['sandybrown'] = '#f4a460'; c['seagreen'] = '#2e8b57'; c['seashell'] = '#fff5ee'; c['sienna'] = '#a0522d'; c['silver'] = '#c0c0c0'; c['skyblue'] = '#87ceeb'; c['slateblue'] = '#6a5acd'; c['slategray'] = '#708090'; c['snow'] = '#fffafa'; c['springgreen'] = '#00ff7f'; c['steelblue'] = '#4682b4';
...
function GV_Define_Garmin_Icons(icon_dir,garmin_icon_set) {
	...
	if (garmin_icon_set == 'mapsource') { garmin_dir = icon_dir+'garmin/mapsource/'; }
	else if (garmin_icon_set == '24x24') { garmin_dir = icon_dir+'garmin/24x24/'; }
		...
		for (var i=0; i<gv_options['map_type_control']['custom'].length; i++) {
			...
			if (custom['url'] || custom['template']) {
				...
				if (custom['background']) {
					if (typeof(custom['background']) != 'object') { custom['background'] = [ custom['background'] ]; }
				...
				if (custom['foreground']) {
					if (typeof(custom['foreground']) != 'object') { custom['foreground'] = [ custom['foreground'] ]; }
...
function GV_MapTypeControl(options) {
	if (!self.gv_map_object && self.gv_options) { gv_map_object = eval(gv_options['map']); } // this really should have been done long ago, but with some OLD code, this may be the best opportunity
...
if (gv_api_version >= 2) {
	...
	GV_MapTypeControl.prototype.initialize = function(this_map) {
		...
		var excluded_maps = []; var excluded_maps_count = 0; var included_maps = []; var included_maps_count = 0;
		var oor_maps = []; // out-of-range
		if (this.excluded_map_types) {
			for (var j=0; j<this.excluded_map_types.length; j++) {
				excluded_maps[this.excluded_map_types[j]] = true; excluded_maps_count += 1;
			}
		}
		if (this.included_map_types) {
			for (var j=0; j<this.included_map_types.length; j++) {
				included_maps[this.included_map_types[j]] = true; included_maps_count += 1;
			}
		}
		...
		if (this.control_style == 'menu') {
			...
			for (var j=0; j<map_types.length; j++) {
				...
				if (map_ok) {
					...
					if (self.gv_options && gv_options['map_type_control'] && gv_options['map_type_control']['custom_title'] && gv_options['map_type_control']['custom_title'][map_types[j]['type']]) { menu_name = gv_options['map_type_control']['custom_title'][map_types[j]['type']]; }
					opt.appendChild(document.createTextNode(menu_name));
					map_selector.appendChild(opt);
					if (this_map.getCurrentMapType() == eval(opt.value)) { map_selector.selectedIndex = map_selector.length - 1; }
				}
			}
...
function CustomWMSLayers(layers) {
	...
	for (var j=0; j<layers.length; j++) {
		...
		if (mi['background']) {
			if (typeof(mi['background']) == 'string') { mi['background'] = [ mi['background'] ]; } // force it into an array
		...
		if (mi['foreground']) {
			if (typeof(mi['foreground']) == 'string') { mi['foreground'] = [ mi['foreground'] ]; } // force it into an array
			for (var j=0; j<mi['foreground'].length; j++) {
				if (eval("self."+mi['foreground'][j]) != undefined) {
					...
					for(var i in fg_layers) {
						if (mi['fg_opacity'] != undefined) { fg_layers[i].getOpacity = function() { return mi['fg_opacity']; } }
...
function CustomTileBasedLayer(opts) {
	...
	var tileLayers = [];
	if (opts['background']) {
		if (typeof(opts['background']) == 'string') { opts['background'] = [ opts['background'] ]; }
		for (var j=0; j<opts['background'].length; j++) {
			if (eval('self.'+opts['background'][j])) {
				var bg_layers = eval(opts['background'][j]+'.getTileLayers()');
				for(var i in bg_layers) {
					if (opts['bg_opacity'] != undefined) { bg_layers[i].getOpacity = function() { return opts['bg_opacity']; } }
					tileLayers.push(bg_layers[i]);
				}
			}
		}
	...
	for (j=0; j<template.length; j++) {
		...
		layer1.getCopyright = function() { return copyright; };
		layer1.layer = long_name; 
		if (tile_function) { eval('layer1.getTileUrl='+tile_function); }
		tileLayers.push(layer1);
	}
	if (opts['foreground']) {
		if (typeof(opts['foreground']) == 'string') { opts['foreground'] = [ opts['foreground'] ]; }
		for (var j=0; j<opts['foreground'].length; j++) {
			if (eval('self.'+opts['foreground'][j])) {
				var fg_layers = eval(opts['foreground'][j]+'.getTileLayers()');
				for(var i in fg_layers) {
					if (opts['fg_opacity'] != undefined) { fg_layers[i].getOpacity = function() { return opts['fg_opacity']; } }
...
if (google_api_version > 0) {
	...
	GV_MapOpacityControl.prototype.initialize = function(map) {
		...
		opacity_selector.title = "Seleziona l'opacità della mappa";
...
function GV_Tracklist_Tooltip_Show(item_div_name,map_div_name) {
	...
	if ($(item_div_name)) { item_div = $(item_div_name); } else { return false; }
	if ($(map_div_name)) { map_div = $(map_div_name); } else { return false; }
	if ($('gv_legend_tooltip')) { tooltip_div = $('gv_legend_tooltip'); }
	else if ($('gv_tracklist_tooltip')) { tooltip_div = $('gv_tracklist_tooltip'); }
	else { return false; }
...
function GV_Tracklist_Tooltip_Hide(item_div_name) {
	...
	if ($(item_div_name)) { item_div = $(item_div_name); }
	if (tooltip_div) {
...
function GV_Load_Markers_From_XML(opts) {
	var map_name = (opts['map'] && typeof(opts['map']) !== 'function') ? opts['map'] : 'gmap'; var map = eval(map_name);
	var array_name = (opts['array']) ? opts['array'] : 'wpts'; var marker_array = eval(array_name);
...
function GV_Reload_Markers_From_All_Files() {
	var map_name = (self.gv_options && gv_options['map']) ? gv_options['map'] : 'gmap'; var map = eval('self.'+map_name);
	var marker_array_name = (self.gv_options && gv_options['marker_array']) ? gv_options['marker_array'] : 'wpts'; var marker_array = eval('self.'+marker_array_name);
	for (var i = 0; i < marker_array.length; i++) {
		if (marker_array[i].dynamic > 0) {
			map.removeOverlay(marker_array[i]);
			if (marker_array[i].label_object) { map.removeOverlay(marker_array[i].label_object); }
		}
	}
	...
	gv_dynamic_file_index = -1; gv_reloading_data = true;
...
function GV_Load_Markers_From_File(file_index) {
	if (file_index != null && (gv_dynamic_file_index == null || gv_dynamic_file_index < 0)) { gv_dynamic_file_index = file_index; }
	if (!self.gv_options || !gv_options['dynamic_data'] || !gv_options['dynamic_data'][gv_dynamic_file_index]) { return false; }
	var map_name = (self.gv_options && gv_options['map']) ? gv_options['map'] : 'gmap'; var map = eval('self.'+map_name);
	var marker_array_name = (self.gv_options && gv_options['marker_array']) ? gv_options['marker_array'] : 'wpts'; var marker_array = (eval('self.'+marker_array_name)) ? eval(marker_array_name) : null;if (!marker_array) { eval(marker_array_name+' = []'); }
	...
	if (opts['url'].match(/^http:\/\/maps\.google\./)) { // Google directions, My Maps, etc.
		...
		if (opts['track_options']) { opts['track_options']['name'] = 'SPOT positions'; }
	...
	if (gv_marker_list_exists) {
		if (gv_dynamic_file_index == 0) { gv_marker_list_div.innerHTML = 'Loading markers...'; gv_marker_list_html = ''; gv_marker_list_folders = []; }
		if (gv_static_marker_count && !self.gv_static_markers_loaded) { // problem: this loads ALL static markers even if "filter:true" is enabled
			for (var s=0; s<marker_array.length; s++) {
				...
				if (typeof(m.list_html) != 'undefined') {
					if (m.folder) { gv_marker_list_folders[m.folder] = (gv_marker_list_folders[m.folder]) ? gv_marker_list_folders[m.folder] + m.list_html : m.list_html; }
					else { gv_marker_list_html += m.list_html; }
				}
			}
...
function GV_Reload_Markers_From_File(index,et_al) {
	if (!index) { index = 0; }
	if (!self.gv_options || !gv_options['dynamic_data'] || !gv_options['dynamic_data'][index]['url']) { return false; }
	var map_name = (self.gv_options && gv_options['map']) ? gv_options['map'] : 'gmap'; var map = eval('self.'+map_name);
	var marker_array_name = (self.gv_options && gv_options['marker_array']) ? gv_options['marker_array'] : 'wpts'; var marker_array = eval('self.'+marker_array_name);
	
	if (self.gv_options && gv_options['marker_filter_options'] && !gv_options['dynamic_data'][index]['reload_on_request']) { gv_options['marker_filter_options']['filter'] = false; gv_filter_waypoints = false; }
	
	var current_bounds = map.getBounds(); var current_center = map.getCenter(); var current_zoom = map.getZoom();
...
function GV_Load_Markers_From_JSON(url) {
	var key = ''; var sheet_id = 'default'; var full_url;
...
function GV_Load_Markers_From_XML_File(url) {
	...
	if (url.indexOf('http') == 0) {
		...
		if (url.indexOf(server) == 0 && !url.match(/csv$/i)) {
			...
			else { proxy_program = 'http://maps.gpsvisualizer.com/google_maps/xml-json.cgi?url='; }
...
function GV_Load_Markers_From_Data_Object(data) {
	if (!gv_options['dynamic_data'][gv_dynamic_file_index]) { return; }
	var opts = gv_options['dynamic_data'][gv_dynamic_file_index];
	...
	map_name = (self.gv_options && gv_options['map']) ? gv_options['map'] : 'gmap'; var map = eval('self.'+map_name);
	if (!map) { return false; }
	marker_array_name = (self.gv_options && gv_options['marker_array']) ? gv_options['marker_array'] : 'wpts'; var marker_array = eval('self.'+marker_array_name);
	if (!marker_array) {
		wpts = []; marker_array = wpts;
		gv_options['marker_array'] = 'wpts';
	}
	track_array_name = (self.gv_options && gv_options['track_array']) ? gv_options['track_array'] : 'trk'; var track_array = eval('self.'+track_array_name);
	track_info_array_name = (self.gv_options && gv_options['track_info_array']) ? gv_options['track_info_array'] : 'trk_info'; var track_info_array = eval('self.'+track_info_array_name);
	track_segments_array_name = (self.gv_options && gv_options['track_segments_array']) ? gv_options['track_segments_array'] : 'trk_segments'; var track_segments_array = eval('self.'+track_segments_array_name);
	if (!self.track_array) {      trk = [];      track_array = trk;           gv_options['track_array'] = 'trk'; }
	if (!self.track_info_array) { trk_info = []; track_info_array = trk_info; gv_options['track_info_array'] = 'trk_info'; }
	if (!self.track_segments_array) { trk_segments = []; track_segments_array = trk_segments; gv_options['track_segments_array'] = 'trk_segments'; }
	
	if (!root_tag) {
		for (var rt in data) { if (!root_tag && data[rt]) { root_tag = rt; } }
	}
		...
		for (var i=0; i<data['features'].length; i++) {
			...
			if (feature['geometry'] && feature['geometry']['coordinates'] && feature['geometry']['coordinates'].length > 1) {
				...
				if (feature['properties']) {
					...
					if (feature['properties']['photoWidth'] && feature['properties']['photoHeight']) { m['photo_size'] = feature['properties']['photoWidth']+','+feature['properties']['photoHeight']; } else if (feature['properties']['photoWidth']) { m['photo_width'] = feature['properties']['photoWidth']; }
	...
	if (root_tag && !marker_tag) {
		if (root_tag == 'feed' && data[root_tag]['entry']) {
			...
			for (var mt in data[root_tag]) { if (!marker_tag && data[root_tag][mt]) { marker_tag = mt; } }
		}
	}
				...
				if (doc['Placemark']) {
					...
					for (var j=0; j<doc['Placemark'].length; j++) {
						...
						if ((pm['Point'] && pm['Point']['coordinates']) || (pm['MultiGeometry'] && pm['MultiGeometry']['Point'] && pm['MultiGeometry']['Point']['coordinates']) || (pm['GeometryCollection'] && pm['GeometryCollection']['Point'] && pm['GeometryCollection']['Point']['coordinates'])) { // WAYPOINT
									...
									if (isNaN(marker_data['lat']) || isNaN(marker_data['lon']) || (marker_data['lat'] == 0 && marker_data['lon'] == 0) || Math.abs(marker_data['lat']) > 90 || Math.abs(marker_data['lon']) > 180 || marker_data['lat'] == undefined || marker_data['lon'] == undefined) {
										...
										if (opts['ignore_styles']) {
											...
											if (pm['Style'] && pm['Style']['IconStyle']) { // local styles override globals
												...
												if (ist['scale']) { marker_data['scale'] = parseFloat(ist['scale']); }
												if (ist['color'] && ist['color'].length == 8) {
												...
												for (var k=0; k<pm['StyleMap']['Pair'].length; k++) {
													if (pm['StyleMap']['Pair'][k]['key'] && pm['StyleMap']['Pair'][k]['key'] == 'normal' && pm['StyleMap']['Pair'][k]['Style']) {
														...
														if (st['IconStyle']) {
															...
															if (ist['Icon'] && ist['Icon']['href']) { marker_data['icon'] = ist['Icon']['href']; }
															if (ist['scale']) { marker_data['scale'] = parseFloat(ist['scale']); }
															if (ist['color'] && ist['color'].length == 8) {
																marker_data['color'] = '#'+ist['color'].replace(/\w\w(\w\w)(\w\w)(\w\w)/,'$3$2$1');
																marker_data['opacity'] = parseInt(ist['color'].replace(/(\w\w)\w\w\w\w\w\w/,'$1'),16)/255;
														...
														if (st['BalloonStyle']) {
															...
															if (balloon_text) { marker_data['desc'] = balloon_text; }
														}
													}
												}
											}
							...
							track_array[tn] = []; track_info_array[tn] = [];
							...
							if (opts['ignore_styles']) {
								...
								if (pm['styleUrl']) { // check for a global style that might be applied
									...
									if (track_styles[style_id]) {
										for (var attr in track_styles[style_id]) { track_info_array[tn][attr] = track_styles[style_id][attr]; }
									}
									if ((pm['Polygon'] || (pm['MultiGeometry'] && pm['MultiGeometry']['Polygon'])) && polygon_styles[style_id]) {
										for (var attr in polygon_styles[style_id]) { track_info_array[tn][attr] = polygon_styles[style_id][attr]; }
									}
								}
								if (pm['Style'] && pm['Style']['LineStyle']) { // local styles override globals
									...
									if (lst['width']) { track_info_array[tn]['width'] = parseFloat(lst['width']); }
								}
							...
							var coord_count = 0;
							var lat_sum = null; var lon_sum = null; var bounds_e = null; var bounds_w = null; var bounds_n = null; var bounds_s = null;
								...
								if (pm['LineString'][k]['coordinates']) {
									...
									for (var l=0; l<coords.length; l++) {
										...
										if (Math.abs(lat) <= 90 && Math.abs(lon) <= 180) {
											...
											if (!bounds_w || lon < bounds_w) { bounds_w = lon; }
											if (!bounds_e || lon > bounds_e) { bounds_e = lon; }
											if (!bounds_s || lat < bounds_s) { bounds_s = lat; }
											if (!bounds_n || lat > bounds_n) { bounds_n = lat; }
							...
							if (bounds_s != null && bounds_n != null && bounds_w != null && bounds_e != null) {
								...
								track_info_array[tn]['bounds']['s'] = bounds_s; track_info_array[tn]['bounds']['n'] = bounds_n;
								track_info_array[tn]['bounds']['w'] = bounds_w; track_info_array[tn]['bounds']['e'] = bounds_e;
							}
			...
			for (var i=marker_start_index; i<marker_end_index; i++) {
				...
				for (var tag in row) {
					if (tag.indexOf(tag_prefix) == 0) {
						...
						if (value != null && typeof(value) != 'object' && value.toString().match(/\S/)) {
							...
							if (field  == 'georss:point' || field  == 'georss_point') {
								...
								try {
									...
									for (var op in extra_marker_list_options) { marker_data[op] = extra_marker_list_options[op]; }
								} catch(error) {}
								...
								if (numbers.length == 1) { marker_data[field] = [numbers[0],numbers[0]]; }
								else if (numbers.length == 2) { marker_data[field] = [numbers[0],numbers[1]]; }
						...
						if (original_field != field && !marker_data[original_field]) { marker_data[original_field] = marker_data[field]; } // for field synthesis using original column names
					}
				}
				...
				if (opts['ignore_styles']) {
					marker_data['color'] = null; marker_data['icon'] = null; marker_data['icon_size'] = null; marker_data['icon_anchor'] = null; marker_data['scale'] = null; marker_data['opacity'] = null; marker_data['rotation'] = null; 
				}
				var marker_ok = true;
				if (isNaN(marker_data['lat']) || isNaN(marker_data['lon']) || (marker_data['lat'] == 0 && marker_data['lon'] == 0) || Math.abs(marker_data['lat']) > 90 || Math.abs(marker_data['lon']) > 180 || marker_data['lat'] == undefined || marker_data['lon'] == undefined) {
					marker_ok = false;
				...
				if (marker_ok) {
					if (opts['default_marker']) {
						for (mdf=0; mdf<marker_default_fields.length; mdf++) {
							if (!marker_data[marker_default_fields[mdf]] && opts['default_marker'][marker_default_fields[mdf]]) { marker_data[marker_default_fields[mdf]] = opts['default_marker'][marker_default_fields[mdf]]; }
						}
					}
					marker_data['dynamic'] = gv_dynamic_file_index + 1; // +1 so we can test for its presence!
					marker_array.push( GV_Marker(map,marker_data) );
...
if (google_api_version >= 2) {
	...
	ELabel.prototype.redraw = function(force) {
		...
		var x_offset = this.pixelOffset.width; x_offset += (this.centered) ? 0 - parseInt(this.div_.clientWidth/2) : (this.left) ? 0-this.iconSize.width-1-this.div_.clientWidth : this.iconSize.width+1;
		var y_offset = this.pixelOffset.height; y_offset += (this.centered) ? this.iconSize.height + 1 : 0 - parseInt(h/2);
...
function GV_Get_Dynamic_Markers(opts) { // opts = options hash
	...
	if (!eval('self.'+array_name)) { eval(array_name+' = [];'); }
